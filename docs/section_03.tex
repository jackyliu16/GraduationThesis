...
for (i = 0; i < priv->hw_params->rx_queues; i++) { // 初始化函数
    ...
    接受端收到了这个信号之后根据前面规定的每隔 1/波特率 秒对于信号线电平进行采样，将电平信号转换成为实际的字符。
