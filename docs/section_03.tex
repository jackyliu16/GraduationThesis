...
static int bcmgenet_init_rx_ring(struct bcmgenet_priv *priv,
    ...
    接受端收到了这个信号之后根据前面规定的每隔1/波特率秒对于信号线电平进行采样，将电平信号转换成为实际的字符。
    ...
    按照 FPM383F 通信协议要求的那样，在串口层面使用小端序，十位帧格式（1位起始位，1位停止位，8位数据位），采用 57600Bps进行数据传输。

    根据 FPM383F 的规定，串口层面传输的字节数据，在链路层面实际上以类似于下文中的数据结构\ref{FPM383F::UARTdataFrame}进行存在，
    在进行发送的时候逐个字节的进行传输。

    在树莓派上提供了一系列与 UART 相关的寄存器，如 UART0, UART1(mini), UART2-5等，考虑到目前 GPIO 口的占用情况，我采用了 GPIO4,5 对应的
